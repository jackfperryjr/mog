    ...
    let count = 1;
        ...
        $.each(characters, function(key, character) {
            if ((character.name.search(regex) != -1)) {
                ...
                output += '<h4 style="color: #2962ff;margin-bottom: -3px;">'+ character.name + '</h4>';
                ...
                output += '<img style="width: 200px; padding: 7px; border: 1px solid #e0e0e0; margin: 20px 0; float: right;" src=' + character.picture + ' alt="Picture of ' + character.name  + '." title="Picture of ' + character.name  + '.">';
                output += '<br/></div>';
                if(count%2 == 0) { 
                    output += '</div>'
                }
